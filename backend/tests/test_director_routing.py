...
def response(stop='stop', text='{"shots": []}', status=200):
...
class DirectorRoutingTests(unittest.TestCase):
    ...
    def test_director_uses_actual_prompt_and_private_provider_route(self):
        with patch.object(llm,'_client') as anthropic:
            self.assertEqual(llm.call_agent(prompts.CINEMATOGRAPHY_AGENT,'approved story'),{'shots':[]})
        ...
        self.assertEqual(request['json']['provider'],{'allow_fallbacks':False,'data_collection':'deny'})
