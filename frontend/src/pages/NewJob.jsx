...
function SelectField({ label, note, value, onChange, children }) {
  return (
    <label className="flex flex-col gap-1 min-w-[132px]">
      ...
      {note && (
        <span className="text-[10px] leading-tight max-w-[150px]" style={{ color: COLORS.muted }}>
          {note}
        </span>
      )}
    </label>
...
export default function NewJob({ onSubmit }) {
  ...
  function handleLanguageChange(event) {
    ...
    if (nextLanguage !== "English" && !NON_ENGLISH_MODELS.has(aiModel)) {
      setAiModel("Seedance 2.5");
    }
  }
  ...
  return (
    <main className="w-full min-h-screen flex items-center justify-center px-4 py-10">
      <form onSubmit={handleSubmit} className="w-full max-w-5xl flex flex-col gap-5">
        <div>
          <p className="text-xs uppercase tracking-[0.2em] mb-2" style={{ color: COLORS.marigold }}>
            Merakify Director
          </p>
          ...
          <p className="text-sm" style={{ color: COLORS.muted }}>
            One sentence is enough. The defaults below are ready to go.
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-start gap-3">
          ...
          <button
            ...
            aria-label="Add reference asset"
            className="mt-5 h-9 w-9 rounded-md flex items-center justify-center"
            style={{
              background: assetPanelOpen ? COLORS.marigold : COLORS.panel,
              border: `1px solid ${assetPanelOpen ? COLORS.marigold : COLORS.border}`,
              color: assetPanelOpen ? COLORS.bg : COLORS.text,
            }}
          >
        ...
        {duration === "Custom" && (
          <input
            ...
            placeholder="Custom duration, e.g. 45 seconds"
            className="w-full max-w-xs rounded-md px-3 py-2 text-sm outline-none"
            style={{ background: COLORS.field, border: `1px solid ${COLORS.border}`, color: COLORS.text }}
          />
        )}
        ...
        {assetPanelOpen && (
          <section className="rounded-xl p-4 flex flex-col gap-4" style={{ background: COLORS.panel, border: `1px solid ${COLORS.border}` }}>
            ...
            {loadingAssets ? (
              <p className="text-sm flex items-center gap-2" style={{ color: COLORS.muted }}>
                ...
                {assets.map((asset) => {
                  ...
                  return (
                    <button
                      ...
                      style={{ border: `2px solid ${selected ? COLORS.marigold : COLORS.border}`, background: COLORS.field }}
                    >
                      <img src={asset.url} alt="" className="w-full h-20 object-cover" />
                      <span className="block text-[10px] px-2 py-1 truncate" style={{ color: COLORS.muted }}>
                        {asset.filename}
                      </span>
                    </button>
                  );
        ...
        {selectedAsset && (
          <div className="flex items-center gap-2 text-xs" style={{ color: COLORS.muted }}>
            ...
            <button type="button" onClick={() => setSelectedAsset(null)} style={{ color: COLORS.marigold }}>
              Remove
            </button>
          </div>
        )}
        ...
        <button
          type="submit"
          disabled={!canSubmit}
          className="self-start px-6 py-3 rounded-md text-sm font-semibold flex items-center gap-2"
          style={{
            background: canSubmit ? COLORS.marigold : COLORS.border,
